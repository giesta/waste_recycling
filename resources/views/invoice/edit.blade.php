...
<div id="contentLeft">
    ...
    <form class="form-horizontal" role="form" method="POST" action="{{ route('confirmEditInvoice', $invoice->id) }}">
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
             <div class="form-group2">
                 <div class="form-group1" style="float:left;">
                    <label class="col-md-4 control-label">Klientas:</label>
                     <div class="col-md-12">
                       <input type="text" class="form-control" name="client_name" value="{{$invoice->client_name}}">
                      </div>
                  </div>
              <div class="form-group1" style="float:left;">
                   <label class="col-md-4 control-label">Adresas:</label>
                   <div class="col-75">
                      <input type="text" class="form-control" name="client_address" value="{{$invoice->client_address}}">
                    </div>
               </div>
</div>
      <div class="form-group2">
        <div class="form-group1" style="float:left;">
            <label class="col-md-4 control-label">Telefono numeris:</label>
                <div class="col-75">
                     <input type="text" class="form-control" name="client_number" value="{{$invoice->client_number}}">
                </div>
        </div>
       <div class="form-group1" style="float:left;">
            <label class="col-md-4 control-label">Kodas:</label>
            <div class="col-75">
                <input type="text" class="form-control" name="client_code" value="{{$invoice->client_code}}">
            </div>
         </div>
</div>
         <div class="form-group2">
          <div class="form-group1" style="float:left;">
            <label class="col-md-4 control-label">Data:</label>
              <div class="col-75">
                <input type="text" name="date" class="form-control" placeholder="Pasirinkti datą" onfocus="(this.type='date')" value="{{$invoice->date}}">
              </div>
</div>
     <div class="form-group1" style="float:left;">
 ...
 @foreach($lines as $line)
   ...
   <td><select class="form-control" name="stock_id[]" id="a0">
       @foreach($stocks as $nt)
         <option value="{{$nt->getKey()}}" {{ $line['stocks']->name == $nt->name ? 'selected="selected"' : '' }} data-price="{{ $nt->price }}">{{$nt->name}} ({{$nt['kinds']->name}})</option>
         @endforeach
    </select>
</td>
   <td><div class="col-md-12">
        <input id="b0" type="number" class="form-control" id ="price" name="price[]" value="{{$line->price}}"  onblur="multiplication()" min="0" step="0.01">
        </div></td>
   <td>
   <div class="col-md-12">
         <input id="c0" type="number" class="form-control" name="amount[]" value="{{$line->amount}}" onblur="multiplication()" min="0">
    </div>
   </td>    
  </tr>
  @endforeach
...
<div align="center" style="float: right;color:black; background-color: white; width: 300px; border-style: solid; margin: auto;
  border: 3px solid black;">
      <p id="p1" name= "total_sum">Viso: {{$invoice->total_sum}} EUR</p> 
      <input id="total_sum" type="hidden" class="form-control" name="total_sum" value="{{$invoice->total_sum}}">
      </div>
      </div> 
